...
const INTERVAL_FOR_UPDATING_BTC_BLOCK_HASH_DATA = Number(process.env.INTERVAL_FOR_UPDATING_BTC_BLOCK_HASH_DATA)
...
exports.handler = async (event: any, context: Context, callback: Callback) => {
  ...
  const startHeight = Number(lastUpdatedBlockHashData['maxBufferedHeight']) + 1
  let height = startHeight
  while (height < latestBlockHashData['height'] && (height - startHeight + 1) <= INTERVAL_FOR_UPDATING_BTC_BLOCK_HASH_DATA) {
    let blockHashData = await Config.getBtcBlockHashData(height.toString(), 0, 500, BtcApiURL)
